...
| B1 — Tangible core | In progress (WP-01, WP-02 complete) | `RealityView` hosting proven in the simulator with the existing MVP suites still green (`make ci-check`); the `box-scene-v1` derivation core is fixture-tested and inside its reducer budget; scene module builds for arm64 device; static audits green | All B1 acceptance rows (SCN/LID/PULL/DIAL/PEEK/MOT/SND/FST/DGR/PRF/AXS), MVP + Share ingestion regression at Q0 and forced Q2, Instruments runs on both reference devices with the oldest-device tier gate met, quality-tier behavior matrix, gesture-equivalence audit, owner experience sign-off |
