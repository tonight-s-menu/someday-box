...
- **Findings (WP-04, 2026-08-17):** RealityKit has no boolean subtraction, so the body is assembled from a floor and four rounded slabs — a single rounded box is solid and buries the stack. Paper size is bounded by the seeded jitter: at maximum offset and yaw the corners must still clear the interior walls. `PhysicallyBasedMaterial` resolves its colour once at creation, so a dynamic `UIColor` freezes at whichever appearance was current — §9.4's appearance response has to be applied explicitly on every change. **Open for WP-06/WP-08:** with the lid open, its shadow reads as a heavy grey smear on the ground; the shadow map needs revisiting when the lid sequences are built.
